...
import { IconEdit, IconLogout, IconMessage, IconStar } from '@tabler/icons-react';
...
const MOCK_USER_DATA: IUserInfo = {
  ...
  isVerified: false,
  language: 'ru',
  avatar: { id: 1, url: 'https://reqres.in/img/faces/1-image.jpg' },
  phoneNumber: '89130000000',
  name: 'Тестовый Пользователь',
  description: 'Это тестовый пользователь для демонстрации',
  stats: { reviews: 15, rating: 4.8, offers: 7 },
};

export const Profile = () => {
  const [user, setUser] = useState<IUserInfo>(MOCK_USER_DATA);
  ...
  const handleSave = () => {
    setUser({ ...user, name: editForm.name, description: editForm.description });
    close();
  };
    ...
    <>
      <Container size="sm" py="xl">
          ...
          <Grid.Col span={4}>
            <Card shadow="sm" padding="md" radius="md" withBorder>
              <Group>
                <IconStar size={24} />
                <div>
                  <Text fw={700} size="xl">{user.stats.rating}</Text>
          ...
          <Grid.Col span={4}>
            <Card shadow="sm" padding="md" radius="md" withBorder>
              <Group>
                <IconMessage size={24} />
                <div>
                  <Text fw={700} size="xl">{user.stats.offers}</Text>
        ...
        <Paper shadow="sm" p="md" radius="md" mb="lg" withBorder>
          ...
          <Stack gap="sm">
            <Group>
              <Text fw={500} w={80}>Логин:</Text>
              <Text>{user.login}</Text>
            </Group>
            <Group>
              <Text fw={500} w={80}>Почта:</Text>
              <Text>{user.email}</Text>
            </Group>
            {user.phoneNumber && (
              <Group>
                <Text fw={500} w={80}>Телефон:</Text>
                <Text>{user.phoneNumber}</Text>
              </Group>
            )}
            <Group>
              <Text fw={500} w={80}>Статус:</Text>
              <Text>{user.isVerified ? 'Подтвержден' : 'Не подтвержден'}</Text>
            </Group>
            <Group>
              <Text fw={500} w={80}>Язык:</Text>
              <Text>{user.language === 'ru' ? 'Русский' : user.language}</Text>
            </Group>
          </Stack>
        </Paper>
