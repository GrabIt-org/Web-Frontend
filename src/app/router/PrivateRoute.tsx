...
export const PrivateRoute: FC<PrivateRouteProps> = ({ children }) => {
  const { isAuthenticated, isLoading, wasUnauthorized, login } = useAuth();
  ...
  if (!isAuthenticated) {
    return (
      <Flex direction="column" align="center" justify="center" style={{ minHeight: '70vh' }} gap="md">
        ...
        <Text size="xl" fw={700}>
          {wasUnauthorized ? 'Вы вышли из аккаунта' : 'Требуется авторизация'}
        </Text>
        <Text c="dimmed" ta="center" style={{ maxWidth: 380 }}>
          {wasUnauthorized
            ? 'Сессия завершилась. Войдите снова, чтобы продолжить.'
            : 'Эта страница доступна только авторизованным пользователям.'}
        </Text>
        <Button mt="sm" size="md" radius="md" bg="#FF8104" onClick={login}>
          Войти
        </Button>
      </Flex>
    );
